...
{
	return openWindow("/forums/sendmessage.php?" + SESSIONURL + "do=im&type=" + imtype + "&userid=" + userid, width, height);
}
...
{
	window.open("/forums/reputation.php?" + SESSIONURL + "p=" + postid, "Reputation", "toolbar=no, scrollbars=yes, resizable=yes, width=400, height=241");
	return false;
}
...
function validatemessage(messageText, subjectText, minLength, maxLength, ishtml, tForm)
{
	...
	//Check for [code] or [php] tags is necessary - By Thaminda Apr 2005
	else if ((messageText.toLowerCase().match(/(&lt;script)|(&lt;?)|(&lt;%)|(&lt;style)|(&lt;applet)|(&lt;?xml)|(&lt;!--)|(&lt;html)/)) && (!messageText.toLowerCase().match(/(\[code\])|(\[php)|(\[highlight=)/)))
	{
		if (confirm('You appear to be posting some code, but have omitted [code] or [php] tags from your message.\n\nClick OK to go back and add them, or cancel to continue without.')) 
			return false;
		else return true;
	}
...
// #############################################################################
// function to disable form after submit
...
// Expand - BBCode
var folder='';
var coll='cp_collapse.gif';
var expd='cp_expand.gif';
function expandit(curobj){
	if(vbDOMtype=='std')
	{
		folder=curobj.nextSibling.nextSibling.style;
	} else {
		folder=document.all[curobj.sourceIndex+2].style;
	}
	...
	if(curobj.src.indexOf(coll)!=-1){
		curobj.src=curobj.src.replace(coll,expd);
	} else {
		curobj.src=curobj.src.replace(expd,coll);
	}
}

/*======================================================================*\
|| ####################################################################
|| # Downloaded: 09:36, Fri Jan 7th 2005
