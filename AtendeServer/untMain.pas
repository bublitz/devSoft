...
type
  TfrmMain = class(TForm)
    ...
    procedure chkEspecial1Click(Sender: TObject);
...
begin
  ...
  if DM.cdsData.Locate('DATA', StrToDate(edtData.Text), []) then
  begin
    ...
    begin
      ...
      DM.cdsData.Post;
      DM.cdsData.SaveToFile(DM.cdsData.FileName);
    end;
  end
  else
  begin
...
begin
  frmPainel.Alarme('000', '000', False);
end;

procedure TfrmMain.chkEspecial1Click(Sender: TObject);
begin
  if frmPainel <> nil then
    frmPainel.Especial1.Checked := not frmPainel.Especial1.Checked;
end;
...
//    Socket.SendText('SE')
//  else
    Socket.SendText('SO') // Servidor Ok
end;
...
//  end;
  ...
  begin
    ...
    begin
      ...
      frmPainel.Alarme(IntToStr(atual), mesa, False);
    end;
  end
  else
  if comando = 'NE' then // Nova Senha Especial
  begin
    if DM.cdsData.Locate('DATA', StrToDate(edtData.Text), []) then
    begin
      atual := DM.cdsDataSENHA_ATUAL.Value + 1;
      ...
      DM.cdsData.SaveToFile(DM.cdsData.FileName);
      Socket.SendText('OK' + IntToStr(atual)); // retorna a senha atual
      frmPainel.Alarme(IntToStr(atual), mesa, True);
    end;
  end
  else
  if comando = 'CS' then // Chama Senha
  begin
    frmPainel.Alarme(senha, mesa, False);
    Socket.SendText('OK' + senha); // retorna a senha atual
  end
  else
  if comando = 'CE' then // Chama Senha Especial
  begin
    frmPainel.Alarme(senha, mesa, True);
    Socket.SendText('OK' + senha); // retorna a senha atual
  end
  else
  ...
  if comando = 'CI' then // Cancela Senha e Chama uma Nova Especial
  begin
    if DM.cdsData.FindKey([data]) then
    begin
      atual := DM.cdsDataSENHA_ATUAL.Value + 1;
