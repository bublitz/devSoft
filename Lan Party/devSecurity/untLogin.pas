...
uses
  ...
  Dialogs, ComCtrls, Buttons, StdCtrls, ExtCtrls, ScktComp, MMSystem;
...
begin
  ...
  begin
    ...
    tempoinic := Time;
    nickname := Copy(mens, 8, Pos('.'#13#13, mens)-8);
    if frmMain.wav_login <> 'Error' then
      SndPlaySound(PChar(frmMain.wav_login), SND_ASYNC);
  ...
  begin
    ...
    if mens = 'NO:02' then
    begin
      if frmMain.wav_error <> 'Error' then
        SndPlaySound(PChar(frmMain.wav_error), SND_ASYNC);
      ShowMessage('Login (usuário e/ou senha) incorreto!'#13'Tente novamente!! ;-(')
    end
    else
    if mens = 'NO:03' then
...
begin
  ...
  StatusBar1.SimpleText := 'Tente novamente....';
end;
