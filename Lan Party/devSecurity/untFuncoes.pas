...
function GeraNomeLogin(nome: string): string;
function iif(exp1: boolean; val1, val2: variant): variant;
...
begin
  ...
  Result := UpperCase(res + nome);
end;

function iif(exp1: boolean; val1, val2: variant): variant;
begin
  if exp1 then Result := val1 else Result := val2;
end;
